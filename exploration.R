...
# what sampling interval do we have?
  schran_temp %>%
    group_by(logger_ID) %>%
    summarise(max_date = max(date),
              min_date = min(date)) %>%
    filter(min_date > "2021-09-01")
    
    # group_by() %>%
    # summarise(mean_maxdate = mean(max_date),
 ...
 ### split Obergurgl plot column into Site, Area, Block and mowing -----
  all_seeds %>%
    ...
    select(-block) -> all_seeds # we can delete blocks as we only have block A
        ...
        all_seeds %>%
          group_by(site) %>%
          summarise(mean_germrate = mean(germ_rate))
...
# min summer temperatures
  sum_temp <- gurgl_temp %>%
    filter(month == 6 | month == 7) %>%
    group_by(site, area, mowing) %>%
    summarise(summer_min = min(temp)) %>%
    full_join(sum_temp)
  
# min annual temperatures
  sum_temp <- gurgl_temp %>%
    group_by(site, area, mowing) %>%
    summarise(ann_min = min(temp)) %>%
    full_join(sum_temp)  
...
# number of snow days
 ...
 ## date of snowmelt
  ...
  ## number of snow free days from Dec-snowmelt
  sum_temp <- gurgl_temp %>%
        left_join(sum_temp) %>%
        filter(day >= "2021-11-01" &
                 day <= last_snowday) %>%
        group_by(site, area, mowing, day) %>%
        summarise(temp_mean = mean(temp),
                  temp_sd = sd(temp)) %>%
        filter(temp_sd >= 0.2 ) %>%
    group_by(site, area, mowing) %>% #, month) %>%
    summarise(snowfree_days = length(temp_sd)) %>%
    full_join(sum_temp) %>%
    mutate(snowfree_days = replace_na(snowfree_days, 0) )# one site has NA snow
  
  sum_temp %>%
    ggplot(aes(x=site, y=snowfree_days))+
    geom_boxplot()
...
# number of frost days per month
  ...
  schran_sumtemp <- schran_temp %>%
    filter(month == 9
      #month == 6 | month == 7 July and June data not in all logers...
      ) %>%
    group_by(logger_ID) %>%
    summarise(summer_mean = mean(temp)) %>%
...
#   ## save R environment
rm(gurgl_temp, schran_temp)

save.image(file = "data/summarized_data_foranalysis.RData")
